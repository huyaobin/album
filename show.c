		...
		unsigned char *rgb_buffer,struct imginfo *imageinfo,int xoffset,int yoffset)
{
	...
	bzero(fbmem, pvinfo->xres * pvinfo->yres * pvinfo->bits_per_pixel/8);
...
linklist show_jpg(linklist last_jpg,int action,char *fbmem,
				  struct fb_var_screeninfo *pvinfo,int a,int b)
{
	if(last_jpg == NULL)
	{
		return NULL;	
	...
	write_lcd(fbmem,pvinfo,rgb_buffer,&jpgfile,a,b);
	return jpg;
}
...
int main(int argc,char **argv)
{
	...
	int a,b;
	printf("Please enter the coordinates of the start\n");
	scanf("%d%d",&a,&b);
	...
	else if(S_ISREG(fileinfo.st_mode))
	{
		if(is_jpg(target))
		{
			...
			show_jpg(jpg_file,CURRENT,fbmem,&vinfo,a,b);
		}
